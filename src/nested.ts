...
export function findQuestion(
    ...
    return questions.reduce(
        (a: Question | null, curr: Question): Question | null =>
            (a = curr.id === id ? curr : a),
        null
    );
}
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (a: number, { points }): number => (a += points),
        0
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (a: number, q: Question): number =>
            (a = q.published ? a + q.points : a),
        0
    );
}
...
export function duplicateQuestionInArray(
    ...
    duplicate.splice(index + 1, 0, duplicateQuestion(newId, questions[index]));
    return duplicate;
}
